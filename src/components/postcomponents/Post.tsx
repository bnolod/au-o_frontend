...
export default function Post ({post, language,preview = false, user, groupView, loadPosts, isAuthorized} :{preview?: boolean, user: User, post:  postEntity, language: 'HU' | 'EN', groupView?: boolean, loadPosts: () => void, isAuthorized?: boolean}) {
    ...
    return (
        <div className="outline-none bg-background rounded-3xl mb-6 flex flex-col shadow-[#00000055] shadow-lg overflow-hidden text-textColor">
            <PostHeader favorite={post.favorite} user={post.user} post={post} postId={post.postId} groupView={groupView} loadPosts={()=>loadPosts()} isAuthorized={isAuthorized}></PostHeader>
