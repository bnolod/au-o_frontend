...
export default function GroupMembersTab({tab, id, validMember} : {tab: "posts" | "members" | "about" | "chat" | "options", id:number, validMember: boolean}) {
    useEffect(() => {
        init()
    }, [tab === "members"])
    const navigate = useNavigate();
    const [users, setUsers] = useState<GroupMemberListResponse>();
    async function init() {
                ...
                }
                {
                    users && users.users.map((user) => (
                        <div className="flex items-center gap-2 p-2">
                            <img src={user.user.profileImg} className="rounded-full w-12 h-12" />
                            <div>
